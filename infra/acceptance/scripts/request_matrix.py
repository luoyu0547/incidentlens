...
def request_cell(base_url: str, route: str, amount: int, index: int) -> dict[str, object]:
    request_id = f"matrix-{route}-{amount}-{index}"
...
def main() -> int:
    ...
    for index, (route, amount) in enumerate(CELLS, 1):
        cell = request_cell(args.url, route, amount, index)
