...
export function LogToolbar({ search, onChange }: LogToolbarProps) {
  return <form className="log-viewer__toolbar" aria-label="日志筛选" onSubmit={(event) => event.preventDefault()}>
    ...
    <label>级别 <select aria-label="日志级别" value={search.levels[0] ?? ''} onChange={(e) => onChange({ levels: e.target.value ? [e.target.value as LogRouteSearch['levels'][number]] : [] })}>
      <option value="">全部</option><option value="trace">trace</option><option value="debug">debug</option><option value="info">info</option><option value="notice">notice</option><option value="warn">warn</option><option value="error">error</option><option value="critical">critical</option>
    </select></label>
    <label>模式 <select aria-label="日志模式" value={search.mode} onChange={(e) => onChange({ mode: e.target.value as LogRouteSearch['mode'] })}><option value="history">历史</option><option value="live">实时</option></select></label>
