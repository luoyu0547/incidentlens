...
function EvidenceItem({ evidence }: { evidence: EvidenceSnippetView }) {
  ...
  return <li><p>{evidence.summary}</p><p>类型：{evidence.evidence_kind}；时间：<Timestamp value={evidence.created_at} /></p><button type="button" onClick={() => setExpanded(true)}>查看已脱敏证据</button>{expanded && (detail.isPending ? <p>正在加载证据…</p> : detail.data ? <pre>{detail.data.content_redacted}</pre> : <p>证据加载失败</p>)}</li>;
}
