...
latest_comparison for the three-strategy comparison API.
"""
...
def test_latest_comparison_returns_only_completed(store: EvaluationRunStore) -> None:
    ...
    run_id1 = store.start("react_no_memory", "all")
    ...
    run_id2 = store.start("memory_unverified", "all")
    store.record(run_id2, {"root_service_expected": "payment-service"})

    comparison = store.latest_comparison(scenario="all")
    assert len(comparison) == 1
    assert comparison[0]["strategy"] == "react_no_memory"
...
def test_latest_comparison_returns_latest_per_strategy(store: EvaluationRunStore) -> None:
    ...
    # First run for react_no_memory
    run_id1 = store.start("react_no_memory", "all")
    store.record(run_id1, {"root_service_expected": "payment-service"})
    store.complete(run_id1, {"root_service_accuracy": 0.5})

    # Second run for react_no_memory (should override the first)
    run_id2 = store.start("react_no_memory", "all")
...
def test_latest_comparison_filters_by_scenario(store: EvaluationRunStore) -> None:
    ...
    run_id1 = store.start("react_no_memory", "payment_delay")
    store.record(run_id1, {"root_service_expected": "payment-service"})
    store.complete(run_id1, {"root_service_accuracy": 0.5})

    run_id2 = store.start("react_no_memory", "db_pool_exhaustion")
    ...
    assert comparison_all[0]["strategy"] == "react_no_memory"
